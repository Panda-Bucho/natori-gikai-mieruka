...
宮城県名取市の市議会議員(21名)の活動(Web発信状況・議会での一般質問の登壇状況)と議会の姿(議員定数・議員報酬・投票率の比較)を可視化する、有権者向けの非公式静的サイトです。有権者の市政・選挙への関心を高め、投票率の向上に資することを目的としています。
...
- **一般質問** (`questions.html`): 議員×定例会の登壇マトリクス(直近5年、市議選マーカー付き)と質問テーマ一覧。議長・副議長・監査委員の在任中セルには役職名、初当選前のセルには「—」を表示。期数/今期/5年計/得票数/得票率の列でソート可能。得票率×質問回数・期数×質問回数・年齢×質問回数の散布図(近似線・R²付き、役職経験者は近似線の計算から除外)と、質問テーマのワードクラウド(全議員/議員別切替、wordcloud2.js、語句ハイライト検索つき)も表示。出典は名取市議会映像配信サイトで、各セルから質問映像へリンク
- **議員定数** (`council.html`): 名取市議会の議員定数(21人)を、人口1000人あたり議員数で全国の市区町村と比較する散布図(両対数、べき乗近似線)。宮城県内/東北6県/全国/全国の市のみ/人口段階(全国市議会議長会の区分)の5グループを切替可能
- **議員報酬** (`salary.html`): 名取市議会議員の報酬月額(議員・副議長・議長)を、全国の市区町村と比較する散布図(横軸対数・べき乗近似線)。宮城県内/東北6県/全国/全国の市のみ/人口段階(全国市議会議長会の区分)の5グループを切替可能
- **投票率** (`turnout.html`): 名取市議会議員選挙(2024年1月執行、投票率35.55%)を東北6県227市町村の直近の議員一般選挙と比較する散布図(横軸対数・べき乗近似線)、投票日当日の気象(降水量・気温)との関係を見る軸切替散布図、全227市町村の出典表。東北6県(市町村)/東北6県の市のみ/宮城県(市町村)/宮城県の市のみの4グループを切替可能(全国一括の公的統計が存在しないため選管公表資料・選挙ドットコムから収集)
...
- 本サイトの開発にはAIコーディング(生成AI)を活用しています。誤りを含む可能性があるため、検証視点のAIによるチェックと、一部データの人手による抽出検査(元データとの突き合わせ)で正確性の確認に努めています。お気づきの点は Issue またはお問い合わせフォームでお知らせください
- 一般質問の要約(AI要約)は全議員に対して同一のプロンプト・同一のモデルで機械的に処理しています。使用しているプロンプトは [`prompts/summary-prompt.md`](prompts/summary-prompt.md) として公開しています
- 議員ご本人からのご要望・ご指摘は、対面で直接お話を伺います。なりすまし防止のため、初回のやり取りは対面に限らせていただきます
...
- [令和7年国勢調査 人口速報集計(総務省統計局)](https://www.stat.go.jp/data/kokusei/2025/index.html)
- [全国市議会議長会「市議会議員定数に関する調査結果」「市議会議員報酬に関する調査結果」](https://www.si-gichokai.jp/research/teisu/index.html)
- [全国町村議会議長会「第71回町村議会実態調査」](https://www.nactva.gr.jp/html/research/index.html)
- 東北6県各市町村選挙管理委員会の公表資料・[選挙ドットコム](https://go2senkyo.com/)(投票率データ)
- [気象庁「過去の気象データ検索」](https://www.data.jma.go.jp/stats/etrn/)(選挙執行日当日の気象データ)
